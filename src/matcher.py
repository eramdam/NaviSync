...
def prompt_user_for_lastfm_match(
    ...
    while True:
        choice = input(f"\n   → Select match [0-{len(matches)}]: ").strip() or '0'
...
def get_lastfm_match_for_navidrome_track(
    ...
    # Auto-accept high-confidence matches (95%+ similarity)
    if fuzzy_matches[0]['combined_score'] >= 95:
        ...
        cache.save_fuzzy_match(
            navidrome_track,
            best_match['lastfm_artist'],
            best_match['lastfm_track']
        )
        return best_match['scrobble_info']
